...
2. Uniswap V3 Development Book — Concentrated Liquidity Math
   https://web.archive.org/web/2023/https://uniswapv3book.com/docs/milestone_1/calculating-liquidity/
...
class PositionData:
    ...
    Fields mirror the Uniswap web interface (app.uniswap.org → Pool → Position):
      - weth_amount / usdt_amount   → "Your liquidity" token balances
    ...
    invested_value_usd: float = 0.0  # Initial deposit value (if known)
    ...
    def from_pool_data(cls, pool_data: Dict, strategy: str = "moderate") -> "PositionData":
        ...
        # Fee tier logic (realistic)
        if "USDC" in [token0, token1] and "USDT" in [token0, token1]:
            fee_tier = 0.0001  # 0.01% for stablecoins
        elif "WETH" in [token0, token1] or "ETH" in [token0, token1]:
            fee_tier = 0.0005  # 0.05% for ETH pairs
        else:
            fee_tier = 0.003   # 0.30% for others
        
        # Generate realistic position ranges based on strategy
        range_strategies = {
            "conservative": {"range_pct": 0.50, "capital": 10000, "description": "Wide range, lower fees, safer"},
            "moderate": {"range_pct": 0.25, "capital": 10000, "description": "Balanced risk/reward"},  
            "aggressive": {"range_pct": 0.10, "capital": 10000, "description": "Narrow range, high fees, risky"}
        }
        ...
        return cls(
            ...
            weth_amount=round(weth_amount, 6) if "WETH" in token0 or "ETH" in token0 else round(stable_amount, 2),
            usdt_amount=round(stable_amount, 2) if "USD" in token1 else round(weth_amount, 6),
    ...
    def from_onchain_data(cls, onchain: Dict, pool_data: Dict) -> "PositionData":
        ...
        return cls(
            ...
            weth_amount=onchain.get("amount0", 0),
            usdt_amount=onchain.get("amount1", 0),
...
class UniswapV3Math:
    ...
    def estimate_fee_apy(
        ...
        """
        Estimate annualised fee yield for an LP position.
...
def generate_position_strategies(
    ...
    investment = position_value if position_value > 0 else 10000.0
...
def analyze_position(position: PositionData) -> Dict:
    """
    ...
    liquidity = math_engine.calculate_liquidity(
        position.weth_amount,
        position.usdt_amount,
    ...
    return {
        ...
        # Token balances
        "token0_amount": position.weth_amount,
        "token1_amount": position.usdt_amount,
        "token0_value_usd": round(position.weth_amount * position.current_price, 2),
        "token1_value_usd": round(position.usdt_amount, 2),
